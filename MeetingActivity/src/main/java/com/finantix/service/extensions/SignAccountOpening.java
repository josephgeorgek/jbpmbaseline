...
public class SignAccountOpening extends RESTWorkItemHandler {

	private static final Logger logger = LoggerFactory.getLogger(SignAccountOpening.class);
	...
	public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
		...
		try {
			...
			Signer requiredDocuments = new Signer();
			

			results = 	processDoc(initiator, fullName, email, phone, packageName, docURL, pageNumber, String.valueOf(workItem.getProcessInstanceId()));
	...
	//	docURL = "https://cdn.hsbc.com.sg/content/dam/hsbc/sg/documents/investment-account-opening-booklet.pdf";
		...
		processDoc(initiator, fullName, email, phone, packageName, docURL, pageNumber,pageNumber );

	}

	public static Map processDoc(String initiator, String fullName, String email, String phone, String packageName,
			String docURL, String pageNumber, String processid) {
		ObjectMapper objectMapper = new ObjectMapper();
		...
		try {
			...
			String loc = null;//packageDocDTO.getLocations().get(0).getId();
			
			List<Location> locations = packageDocDTO.getLocations();
			
			for(Location location: locations) {
				if (loc != null) {
					loc = loc +","+location.getId();
				} 
				else loc = location.getId();
			}
			String redirect = "http://localhost:9090/post.html?taskid="+processid;
			
		//curl -X POST "http://localhost:9090/kie-server/services/rest/server/containers/SignDoc_1.0.0-SNAPSHOT/processes/instances/139/variables" -H  "accept: application/json" -H  "content-type: application/json" -d "{    \"signedOff\" : \"true\"}"
	jsonString = " \n" + "  {\n" + "\"Stakeholders\": [\n" + "    {\n" + "     \"FirstName\": \"eSig: \",\n"
					+ "      \"LastName\": \"" + fullName + "\",\n" + "      \"EmailAddress\": \"" + email + "\",\n"
